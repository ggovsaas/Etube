...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create profile only if user has no profile at all (for fallback/legacy)
    // We don't update existing profiles with listing data anymore
    ...
    // Handle voice note file upload to Cloudinary (before creating listing)
    let finalVoiceNoteUrl = voiceNoteUrl || null;
    ...
    // Create listing with all listing-specific data
    const listing = await prisma.listing.create({
      data: {
        ...
        // Listing-specific information (not shared across listings)
        name,
        neighborhood,
        gender,
        preference,
        ...
        // Social media links (listing-specific)
        onlyfans,
        privacy,
        instagram,
        twitter,
        ...
        // Voice note (listing-specific)
        voiceNoteUrl: finalVoiceNoteUrl,
      }
    });

    // Handle photo uploads (simplified - in production you'd upload to cloud storage)
    // Deduplicate files by name and size to prevent triplication
    const seenFiles = new Set<string>();
    const allMedia: Array<{ file: File; type: string }> = [];
    
    const addFileIfNotSeen = (file: File, type: string) => {
      const fileKey = `${file.name}-${file.size}`;
      if (!seenFiles.has(fileKey)) {
        seenFiles.add(fileKey);
        allMedia.push({ file, type });
      }
    };
    
    photos.forEach(file => addFileIfNotSeen(file, 'GALLERY'));
    galleryMedia.forEach(file => addFileIfNotSeen(file, 'GALLERY'));
    comparisonMedia.forEach(file => addFileIfNotSeen(file, 'COMPARISON'));

    console.log('Total media files to process (after deduplication):', allMedia.length);
    ...
    if (allMedia.length > 0) {
      const mediaPromises = allMedia.slice(0, 10).map(async (mediaItem, index) => {
        try {
          ...
          // Create media record linked to listing (listing-specific media)
