...
interface ListingData {
  ...
  images: { url: string }[];
  userId?: string;
  // Listing-specific fields (not from Profile)
  name?: string;
  neighborhood?: string;
  gender?: string;
  preference?: string;
  ...
  user: {
    ...
    profile?: {
      ...
      description?: string;
      isVerified?: boolean;
      // Keep profile for fallback/legacy data
    };
  };
}
...
function mapListingToProfile(listing: ListingData): ListingProfile {
  try {
    ...
    // Combine all media from listing (listing-specific media)
    const allMedia = [
      ...(listing.media || []),
    ...
    // Parse languages - read from listing first, fallback to profile for legacy data
    let languagesList: string[] = [];
    try {
      const languagesSource = listing.languages || profile.languages;
      if (languagesSource) {
        if (typeof languagesSource === 'string') {
          if (languagesSource.startsWith('[')) {
            languagesList = JSON.parse(languagesSource);
          } else {
            languagesList = languagesSource.split(',').map(l => l.trim()).filter(l => l);
          }
        } else if (Array.isArray(languagesSource)) {
          languagesList = languagesSource;
        }
      }
    } catch (e) {
    ...
    // Parse personality tags - read from listing first, fallback to profile for legacy data
    let personalityTags: string[] = [];
    try {
      const tagsSource = listing.personalityTags || profile.personalityTags;
      if (tagsSource) {
        if (typeof tagsSource === 'string') {
          if (tagsSource.startsWith('[')) {
            personalityTags = JSON.parse(tagsSource);
          } else {
            personalityTags = tagsSource.split(',').map(t => t.trim()).filter(t => t);
          }
        } else if (Array.isArray(tagsSource)) {
          personalityTags = tagsSource;
        }
      }
    } catch (e) {
    ...
    // Build social media links - read from listing first, fallback to profile for legacy data
    const socials: ListingProfile['socials'] = [];
    const onlyfans = listing.onlyfans || profile.onlyfans;
    const instagram = listing.instagram || profile.instagram;
    const twitter = listing.twitter || profile.twitter;
    const tiktok = listing.tiktok || profile.tiktok;
    const snapchat = listing.snapchat || profile.snapchat;
    const telegramChannel = listing.telegramChannel || profile.telegramChannel;
    const whatsappBusiness = listing.whatsappBusiness || profile.whatsappBusiness;
    const manyvids = listing.manyvids || profile.manyvids;
    const chaturbate = listing.chaturbate || profile.chaturbate;
    const myfreecams = listing.myfreecams || profile.myfreecams;
    const livejasmin = listing.livejasmin || profile.livejasmin;
    const linkHubUrl = listing.linkHubUrl || profile.linkHubUrl;
    
    if (onlyfans) socials.push({ platform: 'onlyfans', username: 'OnlyFans', url: onlyfans });
    if (instagram) socials.push({ platform: 'instagram', username: 'Instagram', url: instagram });
    if (twitter) socials.push({ platform: 'twitter', username: 'Twitter', url: twitter });
    if (tiktok) socials.push({ platform: 'tiktok', username: 'TikTok', url: tiktok });
    if (snapchat) socials.push({ platform: 'snapchat', username: 'Snapchat', url: snapchat });
    if (telegramChannel) socials.push({ platform: 'telegram_channel', username: 'Telegram', url: telegramChannel });
    if (whatsappBusiness) socials.push({ platform: 'whatsapp_business', username: 'WhatsApp Business', url: whatsappBusiness });
    if (manyvids) socials.push({ platform: 'manyvids', username: 'ManyVids', url: manyvids });
    if (chaturbate) socials.push({ platform: 'chaturbate', username: 'Chaturbate', url: chaturbate });
    if (myfreecams) socials.push({ platform: 'myfreecams', username: 'MyFreeCams', url: myfreecams });
    if (livejasmin) socials.push({ platform: 'livejasmin', username: 'LiveJasmin', url: livejasmin });
    if (linkHubUrl) socials.push({ platform: 'linkhub', username: 'Links', url: linkHubUrl });
    ...
    const name = listing.name || profile.name || listing.title || 'Anúncio';
    const neighborhood = listing.neighborhood || profile.neighborhood || '';
    const gender = listing.gender || profile.gender || '';
    const weight = listing.weight || profile.weight || '';
    const height = listing.height || profile.height || '';
    const ethnicity = listing.ethnicity || profile.ethnicity || '';
    const eyeColor = listing.eyeColor || profile.eyeColor || '';
    const hairColor = listing.hairColor || profile.hairColor || '';
    const shoeSize = listing.shoeSize || profile.shoeSize || '';
    const tattoos = listing.tattoos || profile.tattoos || '';
    const piercings = listing.piercings || profile.piercings || '';
    const smoker = listing.smoker || profile.smoker || '';
    const bodyType = listing.bodyType || profile.bodyType || '';
    const breastSize = listing.breastSize || profile.breastSize || '';
    const breastType = listing.breastType || profile.breastType || '';
    const whatsappEnabled = listing.whatsappEnabled ?? profile.whatsappEnabled ?? false;
    const telegramEnabled = listing.telegramEnabled ?? profile.telegramEnabled ?? false;
    const voiceNoteUrl = listing.voiceNoteUrl || profile.voiceNoteUrl || null;
    ...
    return {
      ...
      physical: {
        ...
        tattoos,
        piercings,
        smoker: smoker === 'Sim' || smoker === 'Yes' || smoker === 'true',
        languages: languagesList,
        bodyType,
        bustSize: breastSize,
        bustType: (breastType === 'Silicone' ? 'Silicone' : 'Natural') as 'Natural' | 'Silicone',
        personalityTags
      },
